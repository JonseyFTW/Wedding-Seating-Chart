...
export const FloorPlan = () => {
  ...
  const handleWheel = (e) => {
    // Prevent default for passive event handling
    e.stopPropagation();
    setScale((prevScale) => {
  ...
  const handleDragEnd = (event) => {
    ...
    if (table) {
      ...
      const maxX = dimensions.width / scale - tableWidth * scale;
      const maxY = dimensions.height / scale - tableHeight * scale;

      const newX = snapToGrid(table.position.x + delta.x / scale);
      const newY = snapToGrid(table.position.y + delta.y / scale);
    ...
    if (furniture) {
      const maxX = dimensions.width / scale - furniture.size.width * scale;
      const maxY = dimensions.height / scale - furniture.size.height * scale;

      const newX = snapToGrid(furniture.position.x + delta.x / scale);
      const newY = snapToGrid(furniture.position.y + delta.y / scale);
      ...
      >
        ...
        >
          ...
          {currentEvent?.furniture?.map((item) => (
            <Furniture key={item.id} item={item} scale={scale} />
          ))}
        </DndContext>
      </div>
