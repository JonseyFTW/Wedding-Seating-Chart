...
const SortableGuestItem: React.FC<SortableGuestItemProps> = React.memo(({ guest, seatNumber, onRemove, onEdit }) => {
  ...
  return (
    ...
    >
      ...
      <span className="w-8 text-gray-500 font-medium">#{seatNumber}</span>
      <span className="flex-1 truncate font-serif text-[#646E78]">{guest.name}</span>
      <div className="flex items-center gap-1">
        <button
    ...
    >
      ...
      >
        <div className="relative text-center mb-4 p-6 border-b border-[#F4E1B2]">
          ...
          <h3 className="text-2xl font-serif text-[#646E78]">Table {tableNumber}</h3>
          <button
            onClick={onClose}
                ...
                {Array.from({ length: table.seats }, (_, i) => {
                  ...
                  return guest ? (
                    ...
                    <div key={`empty-${seatNumber}`} className="flex items-center gap-3 p-4 bg-gray-50 rounded-lg w-full">
                      <div className="w-4" />
                      <span className="w-8 text-gray-500 font-medium">#{seatNumber}</span>
                      <span className="text-gray-400 font-serif">Empty seat</span>
                    </div>
                  );
                })}
              </SortableContext>
